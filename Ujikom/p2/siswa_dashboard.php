...
if (!$siswa) {
    ...
    // Function to extract class name without level
    function extractClassName($fullClassName) {
        $parts = explode(' ', $fullClassName);
        return implode(' ', array_slice($parts, 1));
    }

    // Extract the class name without level for display
    $display_class = extractClassName($siswa['diterima_di_kelas']);

    // Set the only available class option to the student's full class name
    $kelas_tersedia = [$siswa['diterima_di_kelas']];
}
?>
...
<body>
    ...
    <div class="container">
        ...
        <?php else: ?>
            ...
            <p><strong>Kelas:</strong> <?php echo htmlspecialchars($display_class); ?></p>
                ...
                <form action="lihat_raport.php" method="get">
                    ...
                    <select name="kelas" required>
                        <?php foreach ($kelas_tersedia as $kelas): ?>
                            <option value="<?php echo htmlspecialchars($kelas); ?>" selected>
