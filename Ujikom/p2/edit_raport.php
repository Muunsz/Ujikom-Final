    ...
    try {
        ...
        if (!empty($_FILES['foto']['name'])) {
            ...
            if (move_uploaded_file($foto['tmp_name'], $foto_path)) {
                $stmt = $pdo->prepare("UPDATE siswa SET foto = ? WHERE id = ?");
                $stmt->execute([$foto_path, $siswa_id]);
            } else {
                $error = "Gagal mengunggah foto. Error: " . error_get_last()['message'];
            }
        }
...
$stmt = $pdo->prepare("
    SELECT s.*, na.*, e.nama AS nama_ekstrakurikuler, e.nilai AS nilai_ekstrakurikuler, 
           ...
           w.nama AS nama_wali, w.pekerjaan AS pekerjaan_wali, w.alamat AS alamat_wali, w.nomor_telepon AS nomor_telepon_wali, s.foto as foto
    FROM siswa s
...
<body>
    <div class="container">
        ...
        <form action="" method="post" enctype="multipart/form-data">
            ...
            <?php if (!empty($siswa['foto'])): ?>
                <img src="<?php echo htmlspecialchars($siswa['foto']); ?>" alt="Foto Siswa" style="max-width: 200px; margin-top: 10px;">
            <?php endif; ?>
            ...
            <select id="kemandirian" name="kemandirian" required>
                ...
                <option value="Baik<option value="Baik" <?php echo $siswa['kemandirian'] == 'Baik' ? 'selected' : ''; ?>>Baik</option>
    ...
    <script>
        document.getElementById('tambah-ekstrakurikuler').addEventListener('click', function() {
            ...
            newItem.innerHTML = `
                <input type="text" name="ekstrakurikuler[]" placeholder="Nama Ekstrakurikuler">
                <select name="nilai_ekstrakurikuler[]">
                    <option value="">Pilih Nilai</option>
        ...
        document.getElementById('ekstrakurikuler-container').addEventListener('click', function(event) {
            if (event.target.classList.contains('remove-ekstrakurikuler')) {
